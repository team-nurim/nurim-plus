...
import org.nurim.nurim.domain.dto.community.ReadCountsCommunitiesResponse;
...
public class HomeController {
    ...
    public ResponseEntity<Page<ReadHomeCommunityResponse>> communityReadAll(@PageableDefault(
        ...
        Page<ReadHomeCommunityResponse> communityResponses = homeService.getHomeCommunityList(pageable);
        return ResponseEntity.ok().body(communityResponses);
    }

    @CrossOrigin(origins = "http://localhost:8081")
    @GetMapping("/popularCommunityList")
    @Operation(summary = "조회수 높은 커뮤니티 게시물")
    public ResponseEntity<List<ReadHomeCountsCommunitiesResponse>> getPopularCommunities(){

        PageRequest pageRequest = PageRequest.of(0, 3 ,Sort.by("ViewCounts").descending());

        Page<ReadHomeCountsCommunitiesResponse> popularCommunities = homeService.findPopularCommunities(pageRequest);
        List<ReadHomeCountsCommunitiesResponse> popularCommunityList = popularCommunities.getContent();

        return ResponseEntity.ok(popularCommunityList);
    }
